...
{
        ifstream myfile ("/mnt/pspdata/.init/sheet-DB/"+filename);
        ifstream myfilec ("/mnt/pspdata/.init/sheet-DB/"+filename); //copy of that iterator
	ifstream myfiled ("/mnt/pspdata/.init/sheet-DB/"+filename);
	...
	for(int i=0;i<arr_ca.size();i++)
	{
		...
		{
			ofstream outfile ("/mnt/pspdata/.init/frag-sheet/6frag/frag"+to_string(frag_size)+"_"+to_string(i+1)+"_"+filename);
