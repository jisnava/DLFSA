...
int main()
{
	...
	while(getline(list, filename))
	{
		...
		progress += 1.0/469605.0;
		cout.flush();
		//Progress bar code ends
		process("/mnt/pspdata/.init/helices-DB/" + filename,filename);
	}
...
void process(string filename, string succintfname)
{
	...
	for(int i=0;i<arr_ca.size();i++)
	{
		...
		{
			ofstream outfile ("/mnt/pspdata/.init/frag-helix/5frag/frag"+to_string(frag_size)+"_"+to_string(i+1)+"_"+succintfname);
